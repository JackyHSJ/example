...
import 'package:frechat/widgets/personal_bookkeeping/personal_bookkeeping_withdraw_list_item.dart';
...
// 提現紀錄
class PersonalBookkeepingWithdrawView extends ConsumerStatefulWidget {

  const PersonalBookkeepingWithdrawView({super.key});

  @override
  ConsumerState<PersonalBookkeepingWithdrawView> createState() =>
      _PersonalBookkeepingWithdrawViewState();
}

class _PersonalBookkeepingWithdrawViewState extends ConsumerState<PersonalBookkeepingWithdrawView> with AfterLayoutMixin {

  List<WithdrawListInfo> searchList = [];
  ...
  // 查詢收支明細-Income (9-2)
  Future<void> getWithdrawSearchRecordList() async {
    ...
    final WsWithdrawSearchRecordReq reqBody = WsWithdrawSearchRecordReq.create(
        page: page, size: 20, startTime: startTimeFormatted, endTime: endTimeFormatted
    );

    final WsWithdrawSearchRecordRes res = await ref.read(withdrawWsProvider).wsWithdrawSearchRecord(reqBody,
      ...
      return ListView.builder(
        ...
        itemBuilder: (context, index) {
          if (index < searchList.length) {
            return PersonalBookkeepingWithdrawListItem(
              withdrawListInfo: searchList[index],
            );
          } else {
