...
import 'package:frechat/screens/profile/agent/member_list/agent_contact_list/agent_contact_list_view_model.dart';
...
// 推廣中心 -> 成員列表 -> 推廣人脈列表
//
class AgentContactList extends ConsumerStatefulWidget {
  const AgentContactList({super.key, required this.agentUtil, required this.startTime, required this.endTime});
  final DateTime startTime;
  final DateTime endTime;
  final AgentUtil agentUtil;

  @override
  ConsumerState<AgentContactList> createState() => _AgentContactListState();
}

class _AgentContactListState extends ConsumerState<AgentContactList> with SingleTickerProviderStateMixin, AutomaticKeepAliveClientMixin {
  late AgentContactListViewModel viewModel;
  ...
  void initState() {
    super.initState();
    viewModel = AgentContactListViewModel(ref: ref, setState: setState, context: context);
    ...
    return CustomList.separatedList(
        ...
        children: (context, index){
          return PersonalAgentMemberCell(
            ...
            mode: AgentMemberListMode.primaryPromotor,
          );
        }
    );
  }
  ...
  Widget _searchTextField() {
    final WsAgentMemberListRes? memberList = ref.read(userInfoProvider).agentMemberListFriend;
    return Padding(
