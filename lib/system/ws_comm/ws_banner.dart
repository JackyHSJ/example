...
class BannerWs {
  BannerWs({required this.ref});
  final ProviderRef ref;

  /// Banner資料
  Future<WsBannerInfoRes> wsBannerInfo(
      WsBannerInfoReq wsBannerInfoReq, {
        required Function(String) onConnectSuccess,
        required Function(String) onConnectFail,
      }) async {
    final userInfo = ref.read(userInfoProvider);
    final WsBaseReq msg = WsParamsReq.bannerInfo
      ..tId = userInfo.commToken ?? "emptyToken"
      ..msg = wsBannerInfoReq.toBody();

    /// 傳送ws send
    final res = await WebSocketHandler.sendData(msg,
        funcCode: WsParamsReq.bannerInfo.f,
        onConnectSuccess: (msg) => onConnectSuccess(msg),
        onConnectFail: (errMsg) => onConnectFail(errMsg));

    return res.resultMap == null ? WsBannerInfoRes() : WsBannerInfoRes.fromJson(res.resultMap);
  }
}
